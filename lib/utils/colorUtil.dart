import 'package:flutter/cupertino.dart';

class ColorUtil {
  static Color primary=const Color(0xFFDAAA46);
  static Color themeColor = Color(0xFF33180D);
  static Color themeBlack = Color(0xFF33180D);
  static Color themeWhite = Color(0xFFFFFFFF);
  static Color themeColorBg = Color(0xFFF1F1F1);
  static Color primaryColor = Color(0xFFDAAA46);
  static Color primaryTextColor1 = Color(0xFF8C8C8C);
  static Color primaryTextColor2 = Color(0xFF383838);
  static Color primaryTextColor3 = Color(0xFF717171);
  static Color textFieldBorderColor = Color(0xFFC8C8C8);
  static Color errorTextColor = Color(0xFFF44336);
  static Color rating = Color(0xFFE4BE49);
  static Color grey1 = Color(0xFFB9B9B9);
  static Color grey2 = Color(0xFFA4A4A4);
  static Color thWhite = Color(0xFFffffff);
  static Color thBlack = Color(0xFF000000);
  static Color branchClr = Color(0xFFF4AD23);
  static Color red=const Color(0xffE1433A);
  static EdgeInsets formMargin=const EdgeInsets.only(left:15,right:15,top:10,);
  static Color formTableBorder=Color(0xffA6A6A6);
  static TextStyle formTableBodyTS=TextStyle(fontSize: 13,fontFamily: 'RR',color: themeBlack );
  static TextStyle formTableBodyTSB=TextStyle(fontSize: 13,fontFamily: 'Bold',color: themeBlack );
  static TextStyle formTableHeaderTS=TextStyle(fontSize: 12,fontFamily: 'Med',color:ColorUtil.themeBlack );
  static Color text3=const Color(0xff828282);
  static Color text4=const Color(0xffA6A6A6);
  static Color chkBoxText=const Color(0xff452800);
  static Color disableColor=Color(0xFFe8e8e8);
  static Color avatarBorderColor=Color(0xFFC7D0D8);

}
